...
def test_answer_question_builds_correct_prompt(monkeypatch):
    monkeypatch.setattr(exec_mod, "IndexService", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        exec_mod, "QueryService", lambda *args, **kwargs: DummyQueryService()
    )
    monkeypatch.setattr(
        exec_mod, "OpenAIProvider", lambda *args, **kwargs: DummyOpenAIProvider()
    )
